...
const fetchCommentsByPost = async (postId: number): Promise<void> => {
  // 1. [LOG]: Mensaje inicial con estilo
  console.log(
  ...
  try {
    // 2. [PETICIÓN]: Uso de backticks correcto sin asignación accidental
    const response = await fetch(`${API_URL}/posts/${postId}/comments`);

    // 3. [VALIDACIÓN]: Verificación de estado de respuesta
    if (!response.ok) {
      throw new Error(`Error en la petición: ${response.status} - ${response.statusText}`);
    }

    // 4. [TRADUCCIÓN]: Tipado fuerte con la interfaz Comment
    const data: Comment[] = await response.json();

    // 5. [PROCESAMIENTO]: Informe de resultados
    console.log(`✅ Se recibieron ${data.length} comentarios para el post ID ${postId}.`);

    // 6. [RECORRIDO]: Iteración y verificación de tipado (.email)
    data.forEach(comment => { 
      console.log(`   - Comentario ID ${comment.id} por: ${comment.email}`);
    });

  } catch (error) {
    // 7. [ERRORES]: Captura del error (importante para el debugging)
    console.error("❌ Falló la carga de comentarios:", error instanceof Error ? error.message : error);
  }
};
